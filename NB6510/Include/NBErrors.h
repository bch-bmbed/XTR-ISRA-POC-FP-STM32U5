...
/*
    Summary:
     Function is used to set up customized dynamic memory allocation 
     for the C standard library, namely malloc, calloc, 
     realloc and free.
    Parameters:
     psMemHelper :     [in] structure pointer which holds the 
                       function pointers for customized memory calls.
    Remarks:
     By default SDK will use standard C library memory calls(malloc,free, calloc and realloc).
     If the application have different memory call support(Ex: heap_4.c) then it is
     mandatory to map their calls using NBDeviceCallbackInMemory function before 
     calling NBDevicesInitializeA function.  

     Here is an example custom memory call support using heap_4.c.
     static NBDevicesMemHelper memHelper;
      memHelper.pMalloc = pvPortMalloc;
      memHelper.pCalloc = vPortCalloc;
      memHelper.pRealloc = vPortRealloc;
      memHelper.pFree = vPortFree;
      NBDeviceCallbackInMemory(&memHelper);

    Returns:
     If the function succeeds, the return value is NB_OK. If the
     function fails, one of the error codes is returned (<link Error codes>).
*/
NBResult NB_API NBDeviceCallbackInMemory(const NBDevicesMemHelper * psMemHelper);
